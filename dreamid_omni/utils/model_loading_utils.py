...
def init_fusion_score_model_ovi(rank: int = 0, meta_init=False):
    pkg_root = Path(__file__).resolve().parent.parent  # .../dreamid_omni
    video_config = str((pkg_root / "configs/model/dit/video.json").resolve())
    audio_config = str((pkg_root / "configs/model/dit/audio.json").resolve())
