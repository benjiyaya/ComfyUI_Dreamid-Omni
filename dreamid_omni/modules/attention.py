...
def _normalize_backend_name(name):
    ...
    return mapping.get(normalized, "sdpa")


def set_attention_backend(name):
    global ATTENTION_BACKEND
    requested = _normalize_backend_name(name)
    if requested == "flash_attention" and not (FLASH_ATTN_2_AVAILABLE or FLASH_ATTN_3_AVAILABLE):
        warnings.warn("Flash Attention is unavailable; falling back to SDPA.")
        ATTENTION_BACKEND = "sdpa"
    elif requested == "sage_attention" and not SAGE_ATTN_AVAILABLE:
        warnings.warn("Sage Attention is unavailable; falling back to SDPA.")
        ATTENTION_BACKEND = "sdpa"
    else:
...
def backend_attention(
    ...
    selected = _normalize_backend_name(backend if backend is not None else ATTENTION_BACKEND)

    if selected == "flash_attention":
        if FLASH_ATTN_2_AVAILABLE or FLASH_ATTN_3_AVAILABLE:
            return flash_attention(
                q=q,
                k=k,
    ...
    if selected == "sage_attention":
        if SAGE_ATTN_AVAILABLE:
            q = q if q_scale is None else q * q_scale
            q_t = q.transpose(1, 2).to(dtype)
            k_t = k.transpose(1, 2).to(dtype)
            v_t = v.transpose(1, 2).to(dtype)
            fn = None
            for fn_name in ("sageattn", "attention", "scaled_dot_product_attention"):
                if hasattr(sageattention, fn_name):
                    fn = getattr(sageattention, fn_name)
                    break
            if fn is not None:
                try:
                    out = fn(q_t, k_t, v_t, is_causal=causal, dropout_p=dropout_p, scale=softmax_scale)
                except TypeError:
                    out = fn(q_t, k_t, v_t, causal=causal)
                return out.transpose(1, 2).contiguous()
        warnings.warn("Sage Attention is unavailable or failed; falling back to SDPA.")
        selected = "sdpa"
